...
最近的 1.0.1-r8 迭代在日历视图中为各阶段事件引入可识别的颜色标签，并让 Delivered 页面的月份筛选既控制金额/项目数聚合，也同步筛出匹配月度的已交付项目；月度汇总只依赖真实交付日期或预期交付日期（不再下沉到 updatedAt），同时清理了日历阶段色彩样式，保持旧 localStorage 数据兼容，让交付规划更直观。
...
- 当前正式版基线：`1.0.1`
- 当前页面迭代版：`1.0.1-r8`，继承日历阶段色彩与 Delivered 月份筛选，并修复汇总仅依赖真实交付日期 + 列表同步过滤，同时清理阶段色彩 CSS 并保持旧 localStorage 兼容
- 本地备份目录：`backups/`
...
本次迭代仅会在项目具备至少一条有效的订购明细（名称或数量）时才将其聚合展示，避免无意义空条目干扰视图。

### 8. Delivered 页面
将当前阶段已切换到 Delivered 的项目集中列出，方便快速回顾交付日期、订单状态和日志，并继续编辑或删除已完成项目，减少看板中的视觉干扰。
顶部新增了每月交付金额与项目数聚合卡片，可快速浏览某个月份的交付产值。
